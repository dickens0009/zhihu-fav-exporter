function htmlToMdInOffscreen(html, baseUrl) {
  const container = document.createElement("div");
  container.innerHTML = String(html || "");
  // 关键：offscreen 页面 location.href 是 chrome-extension://...，需要显式传 baseUrl
  return htmlToMarkdown(container, { baseUrl: baseUrl || "" });
}

function extractTextBySelectorInOffscreen(html, selector) {
  const container = document.createElement("div");
  container.innerHTML = String(html || "");
  const el = container.querySelector(selector);
  return (el?.textContent || "").trim();
}

chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  (async () => {
    try {
      if (msg?.type === "OFFSCREEN_CONVERT_HTML_TO_MD") {
        const { html, baseUrl } = msg || {};
        const md = htmlToMdInOffscreen(html, baseUrl);
        sendResponse({ ok: true, md });
        return;
      }

      if (msg?.type === "OFFSCREEN_EXTRACT_TEXT") {
        const { html, selector } = msg || {};
        if (!selector) {
          sendResponse({ ok: false, error: "missing selector" });
          return;
        }
        const text = extractTextBySelectorInOffscreen(html, selector);
        sendResponse({ ok: true, text });
        return;
      }
    } catch (e) {
      sendResponse({ ok: false, error: String(e?.message || e) });
    }
  })();

  return true;
});


